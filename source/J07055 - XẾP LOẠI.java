...
class Student{
    ...
    public double getGPA(){
        return (0.25 * this.sc1 + 0.35 * this.sc2 + 0.4 * this.sc3);
    }
    public String getResult(){
        if(this.getGPA() >= 8) return "GIOI";
        else if(this.getGPA() >= 6.5) return "KHA";
        else if(this.getGPA() >= 5) return "TRUNG BINH";
        return "KEM";
    }
    public String toString(){
        return this.ID + " " + this.name + " " + String.format("%.2f", this.getGPA()) + " " + this.getResult();
    }
}
